...
实时采集 OKX 行情 + 链上数据，落库 PostgreSQL，计算 6 类因子（资金流 / 订单簿 / 衍生品 / 链上 / 市场结构 / 参与者），先用规则引擎做初判，再交给 LangChain Agent 输出严格 JSON Schema 的结构化交易建议。
...
│   ├── data_ingestion/              # OKX WS / REST + 链上 mock + 编排
...
4. 每分钟写一条 mock 链上指标
5. 每 30s 计算一次因子并生成交易信号写入 `signals` 表
...
```json
{
  ...
  "signal": {
    "bias": "long",
    "confidence": 0.62,
    "reason": "net_flow=+820000, ob_imbalance=+0.21, funding=+0.00012, oi_change=+0.0083, trend=uptrend",
    "risk": "Funding may flip if price breaks below 1850; large ask wall at 1880",
    "suggestion": "Scale long around 1862, stop 1849, target 1885 (advisory only)."
  },
  "factors": { "...": "..." }
}
```
...
| 衍生品 | `funding_rate`、`oi_change_pct` |
| 链上 | `net_exchange_flow`、`whale_tx_count`、`gas_fee_gwei`、`burn_rate` |
| 市场结构 | `trend`（HH/HL → uptrend / LH/LL → downtrend / range）、`supports`、`resistances` |
| 参与者 | `whale_active`、`smart_money_bias` |

窗口大小由 `FACTOR_WINDOW_SECONDS` 控制，默认 300 秒。

## 6. 信号生成

1. **规则引擎**（`app/signal_engine/rules.py`）：6 类因子加权打分，输出 `bias / confidence / reason / risk / suggestion`，得分 ∈ [-1, 1]。
2. **LangChain Agent**（`app/signal_engine/llm_agent.py`）：把因子 + 规则打分一起喂给 DeepSeek（OpenAI 兼容协议），用 `with_structured_output(TradingSignal, method="function_calling")` 强制返回严格 JSON。
...
- **真实链上数据**：替换 `MockOnchainProvider` 为 Glassnode / Nansen / Etherscan 客户端（实现同名 `fetch_metrics`）。
