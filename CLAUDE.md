...
- **`app/container.py`** — `AppContainer` dataclass DI container. Wires DB pool, WS/REST clients, factor aggregator, LLM agent, email sender, signal service. Created once in lifespan, accessed via `Depends(get_container)`.
...
- **`app/data_storage/`** — `Database` (asyncpg pool wrapper with transient-error retry) and `Repositories` (all DB access). All writes use `ON CONFLICT DO NOTHING/UPDATE` for idempotency.
...
- **K-lines**: `KlineAggregator` reads raw trades and builds incremental OHLCV bars across 6 timeframes (1m/5m/15m/1h/4h/1d) with `ON CONFLICT DO UPDATE` for live rolling bars.
...
- **Signals**: `SignalService.generate()` checks ATR floor, calls `LLMAgent.analyze()` which throttles via `signals` table timestamps, runs LLM, validates output with `TradingSignal` schema + post-check (RR honesty), then persists.
...
| Group | Key variables |
|-------|--------------|
| Database | `DATABASE_URL`, `DB_POOL_MIN/MAX_SIZE` |
| OKX | `OKX_WS_URL`, `OKX_REST_URL`, `SYMBOLS` (comma-separated) |
| DeepSeek LLM | `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`, `DEEPSEEK_THINKING_ENABLED`, `LLM_TIMEOUT`, `LLM_MIN_INTERVAL_SECONDS` |
| Factors | `FACTOR_WINDOW_SECONDS` (≥1200), `MTF_LOOKBACK_BARS` |
...
- **Thinking mode**: DeepSeek's reasoning_content (chain-of-thought) is captured via a custom `ChatOpenAI` subclass that patches `_create_chat_result`. Stored in `signals.reasoning_content` for audit only.
...
- **Output validation**: `TradingSignal` pydantic model enforces price order (long: sl < entry < tp, short: reversed), minimum 2 take-profit levels, RR > 0. Post-check verifies RR self-report accuracy within 5%.
